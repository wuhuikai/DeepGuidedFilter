...
class MonodepthModel(object):
    ...
    def conv(self, x, num_out_layers, kernel_size, stride, activation_fn=tf.nn.elu, reuse=False, scope=None):
        p = np.floor((kernel_size - 1) / 2).astype(np.int32)
        p_x = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]])
        return slim.conv2d(p_x, num_out_layers, kernel_size, stride, 'VALID', activation_fn=activation_fn, reuse=reuse, scope=scope)
    ...
    def build_vgg(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.params.guided_filter:
                self.disp1 = self.conv(iconv1, 2, 3, 1, None)
            else:
    ...
    def build_model(self):
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.elu):
            with tf.variable_scope('model', reuse=self.reuse_variables):
                ...
                if self.params.guided_filter:
                    with tf.variable_scope('guided_filter'):
                        g = self.conv(self.left, 64, 1, 1, tf.nn.relu, scope='conv1')
                        g = self.conv(g, 1, 1, 1, None, scope='conv2')

                        if self.mode == 'train':
                            g1 = self.conv(self.right, 64, 1, 1, tf.nn.relu, reuse=True, scope='conv1')
