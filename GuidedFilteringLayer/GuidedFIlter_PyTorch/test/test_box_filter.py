...
from guided_filter.box_filter import DiffX, DiffY, BoxFilter

## DiffX
print('DiffX:')
# backward | check grad
test = gradcheck(DiffX(3), (Variable(torch.rand((3, 5, 8, 9)).double().cuda(), requires_grad=True),
                            ), eps=1e-6, atol=1e-4)
...
print('DiffY:')
# backward | check grad
test = gradcheck(DiffY(3), (Variable(torch.rand((3, 5, 8, 9)).double().cuda(), requires_grad=True),
                            ), eps=1e-6, atol=1e-4)
print('\tGrad Check Result:', test)
...
assert np.isclose(y.std(), 475.2433, 1e-4)
...
assert np.isclose(r[:,:,0].mean(),  10305,   1)
...
# backward | check grad
test = gradcheck(BoxFilter(3), (Variable(torch.rand((3, 5, 8, 9)).double().cuda(), requires_grad=True),
                            ), eps=1e-6, atol=1e-4)
print('\tGrad Check Result:', test)