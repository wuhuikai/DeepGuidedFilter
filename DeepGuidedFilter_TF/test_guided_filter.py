...
x_w, x_h = rgb.shape[:2]
w, h = x_w // 8, x_h // 8
hr_gt = resize(resize(gt, (w, h), order=0, mode='reflect'), (x_w, x_h), order=0, mode='reflect')
x, y = [tf.constant(i.transpose((2, 0, 1))[None]) for i in [rgb, hr_gt]]
output = fast_guided_filter(x, y, 64, 0)
...
imsave('test/r.jpg', r)
imsave('test/4_in_1.jpg', np.asarray(np.vstack([
    np.hstack([rgb, hr_gt]),
