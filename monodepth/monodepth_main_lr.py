...
def train(params):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        opt1 = tf.train.AdamOptimizer(args.learning_rate/4)
        opt2 = tf.train.AdamOptimizer(args.learning_rate)
        ...
        tower_grads = []
        tower_losses = []
        backbone_var, guided_filter_var = None, None
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for i in range(args.num_gpus):
                with tf.device('/gpu:%d' % i):
                    ...
                    if backbone_var is None:
                        backbone_var = [v for v in tf.global_variables() if 'guided_filter' not in v.name and 'model' in v.name]
                        guided_filter_var = [v for v in tf.global_variables() if 'guided_filter' in v.name]

                    grads = tf.gradients(loss, backbone_var+guided_filter_var)

                    tower_grads.append(grads)

        grads = [tf.reduce_mean(tf.concat(axis=0, values=[tf.expand_dims(g, 0) for g in grad]), 0)
                 for grad in zip(*tower_grads)]

        grads1 = grads[:len(backbone_var)]
        grads2 = grads[len(backbone_var):]
        train_op1 = opt1.apply_gradients(zip(grads1, backbone_var), global_step=global_step)
        train_op2 = opt2.apply_gradients(zip(grads2, guided_filter_var), global_step=global_step)
        ...
        if args.checkpoint_path != '':
            tf.train.Saver(var_list=backbone_var).restore(sess, args.checkpoint_path.split(".")[0])
...
def test(params):
    ...
    np.save(output_directory + '/disparities_dgf_r_{}_eps_{}.npy'.format(params.dgf_r, params.dgf_eps), disparities)
    np.save(output_directory + '/disparities_pp_dgf_r_{}_eps_{}.npy'.format(params.dgf_r, params.dgf_eps), disparities_pp)
