...
for idx, i in enumerate(img_list):
    ...
    output = F.softmax(interp(output)).cpu().data[0].numpy()
    output = output[:, :img_temp.shape[0], :img_temp.shape[1]]

    if CRF:
        im_crf = cv2.imread(os.path.join(im_path, i[:-1] + '.jpg'))
        d = dcrf.DenseCRF2D(im_crf.shape[1], im_crf.shape[0], 21)

        d.setUnaryEnergy(unary_from_softmax(output, scale=None, clip=1e-5))

        d.addPairwiseGaussian(sxy=(3, 3), compat=3, kernel=dcrf.DIAG_KERNEL, normalization=dcrf.NORMALIZE_SYMMETRIC)
        d.addPairwiseBilateral(sxy=(121, 121), srgb=(5, 5, 5), rgbim=im_crf, compat=4, kernel=dcrf.DIAG_KERNEL,
                               normalization=dcrf.NORMALIZE_SYMMETRIC)

        output = np.reshape(np.array(d.inference(5)), output.shape)
